...
export async function POST(req: NextRequest) {
  ...
  await safeTrigger(roomChannel(roomCode), PUSHER_EVENTS.VOTE_UPDATE, { votes: room.votes });

  if (allVotesSubmitted(room)) {
    const voteResult = processVotes(room);
    if (voteResult.eliminatedPlayerId) room.players[voteResult.eliminatedPlayerId].isAlive = false;
    ...
    if (room.phase === 'game-over') {
      ...
      await safeTrigger(roomChannel(roomCode), PUSHER_EVENTS.PHASE_CHANGED, { phase: 'vote-result', round: room.round, voteResult });
    }
  } else {
    await setRoom(room);
  }
